...
void InitGame(void)
{
    ...
    for (int i = 0; i < MAX_BIG_METEORS; i++)
    {
        ...
        bigMeteor[i].position = (Vector2){(float)posx, (float)posy};
        ...
        bigMeteor[i].speed = (Vector2){(float)velx, (float)vely};
        ...
        if (!pause)
        {
            ...
            if (IsKeyPressed(KEY_SPACE))
            {
                for (int i = 0; i < PLAYER_MAX_SHOOTS; i++)
                {
                    if (!shoot[i].active)
                    {
                        shoot[i].position = (Vector2){ player.position.x + sinf(player.rotation*DEG2RAD)*(shipHeight), player.position.y - cosf(player.rotation*DEG2RAD)*(shipHeight) };
                        shoot[i].active = true;
                        shoot[i].speed.x = 1.5*sinf(player.rotation*DEG2RAD)*PLAYER_SPEED;
                        shoot[i].speed.y = 1.5*cosf(player.rotation*DEG2RAD)*PLAYER_SPEED;
                        ...
                        if (bigMeteor[a].active && CheckCollisionCircles(shoot[i].position, shoot[i].radius, bigMeteor[a].position, bigMeteor[a].radius))
                        {
                            ...
                            for (int j = 0; j < 2; j ++)
                            {
                                if (midMeteorsCount%2 == 0)
                                {
                                    ...
                                    mediumMeteor[midMeteorsCount].speed = (Vector2){cosf(shoot[i].rotation*DEG2RAD)*METEORS_SPEED*-1, sinf(shoot[i].rotation*DEG2RAD)*METEORS_SPEED*-1};
                                }
                                else
                                {
                                    mediumMeteor[midMeteorsCount].position = (Vector2){bigMeteor[a].position.x, bigMeteor[a].position.y};
                                    mediumMeteor[midMeteorsCount].speed = (Vector2){cosf(shoot[i].rotation*DEG2RAD)*METEORS_SPEED, sinf(shoot[i].rotation*DEG2RAD)*METEORS_SPEED};
                                }
                        ...
                        if (mediumMeteor[b].active && CheckCollisionCircles(shoot[i].position, shoot[i].radius, mediumMeteor[b].position, mediumMeteor[b].radius))
                        {
                            ...
                            for (int j = 0; j < 2; j ++)
                            {
                                 if (smallMeteorsCount%2 == 0)
                                {
                                    ...
                                    smallMeteor[smallMeteorsCount].speed = (Vector2){cosf(shoot[i].rotation*DEG2RAD)*METEORS_SPEED*-1, sinf(shoot[i].rotation*DEG2RAD)*METEORS_SPEED*-1};
                                }
                                else
                                {
                                    smallMeteor[smallMeteorsCount].position = (Vector2){mediumMeteor[b].position.x, mediumMeteor[b].position.y};
                                    smallMeteor[smallMeteorsCount].speed = (Vector2){cosf(shoot[i].rotation*DEG2RAD)*METEORS_SPEED, sinf(shoot[i].rotation*DEG2RAD)*METEORS_SPEED};
                                }
